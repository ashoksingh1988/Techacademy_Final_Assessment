...
/**
 * Enhanced ReportManager for Selenium - Comprehensive ExtentReports with Performance Metrics
...
public class EnhancedReportManager {
    ...
    private static final String REPORT_NAME = "Selenium_Web_Automation_Report";
    ...
    public static synchronized void initializeReports() {
        if (extentReports == null) {
            logger.info("Initializing Enhanced Selenium ExtentReports with Performance Tracking");
            ...
            logger.info("Enhanced Selenium ExtentReports initialized successfully: {}", reportPath);
        }
    }
    ...
    private static void configureEnhancedSparkReporter(ExtentSparkReporter sparkReporter) {
        sparkReporter.config().setDocumentTitle("Enhanced Selenium Web Automation Report");
        sparkReporter.config().setReportName("Web Application Test Results - Lead Review");
        ...
        sparkReporter.config().setCss(
            ...
            ".web-metrics { background-color: #e8f4fd; padding: 10px; border-radius: 5px; }"
        );
        
        // Custom JavaScript for enhanced functionality
        sparkReporter.config().setJs(
            "document.addEventListener('DOMContentLoaded', function() {" +
            "  console.log('Enhanced Selenium Report Loaded');" +
    ...
    private static void setEnhancedSystemInformation() {
        ...
        extentReports.setSystemInfo("Test Environment", getEnvironment());
        extentReports.setSystemInfo("Browser", getBrowserInfo());
        extentReports.setSystemInfo("Selenium Hub", getSeleniumHub());
        extentReports.setSystemInfo("Base URL", getBaseUrl());
        ...
        extentReports.setSystemInfo("Framework", "Selenium WebDriver with Enhanced ExtentReports");
    }
    ...
    public static ExtentTest createTest(String testName, String description) {
        ...
        totalTests++;
        logger.info("Enhanced Selenium test created: {} (Execution #{}) ", testName, testExecutionCounts.get(testName));
        return test;
    }
    ...
    public static void logStep(String stepDescription) {
        ...
        if (test != null) {
            ...
            logger.info("Selenium test step logged: {}", stepDescription);
        }
    }
    ...
    /**
     * Logs web-specific performance metrics
     */
    public static void logWebPerformanceMetrics(String action, long loadTime, String url) {
        ExtentTest test = extentTest.get();
        if (test != null) {
            String performanceClass = getPerformanceClass(loadTime);
            
            test.info("<div class='web-metrics'>" +
                     "<h4>🌐 Web Performance Metrics:</h4>" +
                     "<p><strong>Action:</strong> " + action + "</p>" +
                     "<p><strong>URL:</strong> " + url + "</p>" +
                     "<p><strong>Load Time:</strong> <span class='" + performanceClass + " performance-metric'>" + loadTime + "</span>ms</p>" +
                     "<p><strong>Performance Level:</strong> " + getPerformanceLevel(loadTime) + "</p>" +
                     "</div>");
        }
    }
     ...
     */
    private static String getPerformanceLevel(long duration) {
        if (duration < 1000) return "🚀 Excellent";
        if (duration < 3000) return "✅ Good";
        if (duration < 5000) return "⚠️ Acceptable";
        return "❌ Poor";
    }
    ...
    public static synchronized void flushReports() {
        if (extentReports != null) {
            ...
            summaryTest.info("<div style='background-color: #f8f9fa; padding: 15px; border-radius: 5px;'>" +
                           "<h3>📈 Selenium Test Execution Summary</h3>" +
            ...
            extentReports.flush();
            logger.info("Enhanced Selenium ExtentReports flushed successfully with {} total tests", totalTests);
    ...
    private static String getEnvironment() {
        return System.getProperty("test.environment", "qa");
    }

    private static String getBrowserInfo() {
        return System.getProperty("browser.type", "chrome");
    }

    private static String getSeleniumHub() {
        return System.getProperty("selenium.hub", "http://localhost:4444");
    }
